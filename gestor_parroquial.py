...
class AplicacionPrincipal:
    ...
    def _get_part_data(self):
        ...
        for key, widget in self._part_entries.items():
            if isinstance(widget, tuple) and len(widget) == 3:
                d = widget[0].get().strip()
                m = widget[1].get().strip()
                a = widget[2].get().strip()
                if d and m and a:
                    data[key] = f"{d.zfill(2)}/{m.zfill(2)}/{a.zfill(2)}"
                else:
                    data[key] = ""
    ...
    def _generar_inex_html(self, datos):
        ...
        hoy = datetime.now()
        e = html.escape
        tipo = e(datos.get('tipo_sacramento', 'Bautismo'))
        fin = e(datos.get('fin_certificado', 'LO QUE CORRESPONDA'))
        ministro = e((datos.get('ministro', '') or '').strip().upper())
        nombre = e((datos.get('nombre', '') or '').strip().upper())

        fecha_nacimiento = e(self._fecha_eclesiastica(datos.get('fecha_nacimiento', '')))
        libros = e(str(datos.get('libros_buscados', '') or ''))
        anio_desde = e(str(datos.get('anio_desde', '') or ''))
        anio_hasta = e(str(datos.get('anio_hasta', '') or ''))
        ...
        t1_ci = e(str(datos.get('testigo1_ci', '') or ''))
        t1_parentesco = e(str(datos.get('testigo1_parentesco', '') or ''))
        t2_nombre = e((datos.get('testigo2_nombre', '') or '').strip().upper())
        t2_ci = e(str(datos.get('testigo2_ci', '') or ''))
        t2_parentesco = e(str(datos.get('testigo2_parentesco', '') or ''))
    ...
    <style>
            ...
            <tr>
                <td style="border-top:3px double black;border-bottom:3px double black;padding:8px 0;text-align:center;">
                    <div style="font-family:Impact,sans-serif;font-size:18pt;letter-spacing:1px;">
                        CONSTANCIA DE INEXISTENCIA DE CERTIFICADO DE {tipo.upper()}
                    </div>
                </td>
            </tr>
        </table>
        ...
        <p style="text-align:center;font-family:Tahoma;font-size:11pt;margin:12px 0 4px 0;">
            El Presbítero <b>{ministro}</b>, Párroco de esta parroquia,
        </p>
        ...
        <div style="display:flex;justify-content:center;gap:8px;margin:8px 0;font-family:Tahoma;font-size:11pt;">
            <span>en los libros de {tipo.lower()} No.</span>
        ...
        <p style="text-align:center;font-style:italic;font-family:Tahoma;font-size:11pt;margin:16px 0;">
            *** La Certificación de {tipo.lower()} solicitada no se encontró. ***
        </p>
    ...
    def _tabla_cert(self):
        tablas_validas = {
            "Bautismo": "certificados_bautismo",
            "Comunion": "certificados_comunion",
            "Confirmacion": "certificados_confirmacion",
            "Matrimonio": "certificados_matrimonio",
            "Defuncion": "certificados_defuncion"
        }
        tbl = tablas_validas.get(self._cert_tipo_actual)
        if not tbl:
            raise ValueError(f"Tipo de certificado no válido: {self._cert_tipo_actual}")
        return tbl
...
<div class="signatures">
  ...
  <div class="sig"><div class="sig-line"></div><div class="sig-name">Auditor Parroquial</div><div class="sig-title">Consejo de Asuntos Economicos</div></div>
    ...
    def _cambiar_pw(self):
        ...
        valida, msg_pw = self._pw_valida(nueva)
        if not valida:
            Notificacion(self.root, msg_pw, "aviso")
            return
        if nueva != confirmar:
    ...
    def _reset_db(self):
        if self.rol != 'superusuario':
            ...
            return
        pw = simpledialog.askstring("Confirmar", "Escriba su contraseña para restablecer:", show="●", parent=self.root)
        if not pw:
            return
        try:
            with db_cursor() as (c, con):
            ...
            Notificacion(self.root, "Error al verificar contraseña", "error")
            return
        if not messagebox.askyesno("Confirmar", "¿RESTABLECER toda la base de datos? Se perderán todos los datos."):
            return
        if not messagebox.askyesno("Confirmar (2/2)", "Esta acción es IRREVERSIBLE. ¿Continúa?"):
            return
        try:
